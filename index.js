...
function manipulateUrl(url) {
  ...
  myUrl.host = "newhost.com";
  myUrl.searchParams.append("newParam", "newValue");
  myUrl.searchParams.delete("oldParam");
  return {
    href: myUrl.href,
    ...
    search: myUrl.search,
    params: Array.from(myUrl.searchParams.entries()),
  };
...
function searchParamsURL(url) {
  ...
  const newObj = newUrl.searchParams;
  const dictionary = new Map();
  for (const params of newObj) {
    dictionary.set(params[0], params[1]);
  }
  return dictionary;
...
function checkURLParameters(url, params) {
  ...
  for (let key of params) {
    newObj[key] = myUrl.searchParams.has(key);
  }
  return newObj;
...
function manipulateQuery(url, options) {
  ...
  if (options.has("append")) {
    const appendParam = options.get("append");
    for (const [key, value] of options.get("append")) {
      myUrl.searchParams.append(key, value);
    }
  }
  if (options.has("delete")) {
    for (const [key, value] of options.get("delete")) {
      myUrl.searchParams.delete(key, value);
    }
  }
  return myUrl.toString();
  // Створюємо новий об'єкт URL з вхідної URL-адреси.
...
function sortUrlParams(url) {
  const myUrl = new URL(url);
  const entry = [...myUrl.searchParams.entries()];
  entry.sort((a, b) => b[1] - a[0]);
  myUrl.search = "";
  myUrl.searchParams.append(entry[1], entry[0]);
  return myUrl.toString();
  // Створюємо новий об'єкт URL з вхідної URL-адреси.
...
function getURLValues(url) {
  const myUrl = new URL(url);
  const URLSearchParams = myUrl.searchParams;
  const keys = [...URLSearchParams.keys()];
  const array = [];
  for (let key of keys) {
    URLSearchParams.getAll(key);
    array.push(...URLSearchParams.getAll(key));
  }
  return array;
